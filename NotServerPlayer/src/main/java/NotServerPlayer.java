...
//    }
    ...
    public void setRefreshId(int refreshId) {
        if (lastRefreshId != refreshId) {
            ...
            helpers = new HashMap<ProxiedPlayer, NotServerPlayerHelper>();
            
            for(ProxiedPlayer p : BungeeCord.getInstance().getPlayers()) {
                helpers.put(p, new NotServerPlayerHelper(p,0));
            }
            
            for (NotServerPlayerHelper helper : helpers.values()) {
                helper.refresh();
            }
        }
    }
    ...
    public String getText(Short ping) {
        ...
        NotServerPlayerHelper helper = helpers.get(lastPlayer);
